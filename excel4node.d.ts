// source: https://github.com/Dominic-Preap/excel4node/blob/master/typings/excel4node.d.ts
declare module 'excel4node' {
  ...
  class Worksheet {
     ...
     * All options are optional with exception of sqref.
     */
    addDataValidation(opt: WorksheetValidation): void;
    /**
     * Conditional formatting adds custom formats in response to cell reference state.
     * A subset of conditional formatting features is currently supported by excel4node.
    ...
    /**
     * The cell method accesses a single cell or range of cells to manipulate cell method
     * takes two required parameters and 3 optional parameters
     */
  ...
  export class Workbook {
    ...
    /**
     * Sets which tab will be selected when the WorkBook is opened.Accepts Sheet ID
     * (1-indexed sheet in order that sheets were added).
     */
    ...
    /**
     * The writeToBuffer() method access no parameters and returns a promise that resolves with
     * the nodebuffer generated by the JSZip library. This buffer can then be sent to other streams.
     */
    writeToBuffer(): Promise<Buffer>;
  }
  /**
   * Accepts cell reference (i.e. 'A1') and returns object with corresponding row and column.
   * (i.e. returns { row: 5, col: 2})
   */
  ...
  interface WorkbookOption {
    ...
    workbookView?: {
      ...
      activeTab?: number;
      /**
       * Specifies a boolean value that indicates whether to group dates when presenting the user with
  ...
  interface WorksheetOption {
    ...
    pageSetup?: {
      ...
      /**
       * Value must a positive Float immediately followed by unit of measure
       * from list mm, cm, in, pt, pc, pi. i.e. '10.5cm'
       */
      paperHeight?: string;
      /**
       * see lib/types/paperSize.js for all types and descriptions of types.
       * setting paperSize overrides paperHeight and paperWidth settings
       */
      paperSize?: PaperSize;
      /**
       * Value must a positive Float immediately followed by unit of measure
    ...
    sheetView?: {
      ...
      pane?: {
         ...
         * If the pane is frozen, this value indicates the number of columns visible in the top pane.
         */
        xSplit?: number;
        /**
         * Vertical position of the split, in 1/20th of a point; 0 (zero) if none.
         * If the pane is frozen, this value indicates the number of rows visible in the left pane.
         */
        ySplit?: number;
      };
      /**
    ...
    outline?: {
       ...
       * when applying an outline/grouping.
       */
      summaryBelow?: boolean;
      /**
       * Flag indicating whether summary columns appear to the right of detail in an outline,
       * when applying an outline/grouping.
       */
      summaryRight?: boolean;
    };
    /**
     * Flag indicating whether to remove the "spans" attribute on row definitions.
     * Including spans in an optimization for Excel file readers but is not necessary
     */
  ...
  interface WorksheetValidation {
    ...
    /**
     * Restricts input to a specific set of characters. One of 'noControl', 'off', 'on', 'disabled',
     * 'hiragana', 'fullKatakana', 'halfKatakana', 'fullAlpha', 'halfAlpha', 'fullHangul', 'halfHangul'
     */
    imeMode?:
      | 'noControl'
      | 'off'
      ...
      | 'halfHangul';
    /**
     * Must be one of 'between', 'notBetween', 'equal', 'notEqual', 'lessThan', 'lessThanOrEqual',
     ...
     * Defaults to true for type:list
     */
  ...
  interface RowColumnBase {
    ...
    /**
     * Add Filters to a row If not options are given to the filter function,
     * a filter will be added to all columns that contain data.
     * Optionally, if you wish to restrict your filter to a specific range,
     * you can specify start and end rows and columns for that range.
     * The filter row should be included in this range.
     */
  ...
  interface Cell {
    ...
    /**
     * accepts the same object as when creating a new style. When applied to a cell that already has style formatting,
     * the original formatting will be kept and updated with the changes sent to the style function.
     */
   ...
   */
  type BorderStyle =  'none'| 'thin'| 'medium'| 'dashed'| 'dotted'| 'thick'| 'double'| 'hair'| 'mediumDashed'| 
